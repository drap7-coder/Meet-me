...
export function SelectedFiltersPanel({ busy = false, onSearch }: Props) {
  ...
  const panelClass = onPage
    ? "rounded-[18px] border border-line/80 bg-paper p-3.5 shadow-soft sm:p-4"
    : "rounded-[18px] border border-white/12 bg-white/[0.04] p-3.5 backdrop-blur-sm sm:p-4";
  const previewClass = onPage ? "text-sm font-medium leading-6 text-slate/80" : "text-sm font-medium leading-6 text-white/72";
  ...
  const pillClass = onPage
    ...
    : "inline-flex items-center gap-1.5 rounded-full border border-white/14 bg-white/[0.07] px-3 py-1.5 text-xs font-bold text-white/88";
    ...
    <section className={panelClass} aria-label="Selected filters">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="grid min-w-0 flex-1 gap-2.5">
          <HeroSectionLabel onPage={onPage} subtle={!onPage}>
            Selected filters
          </HeroSectionLabel>
