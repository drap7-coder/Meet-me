...
export const parserProvider: ParserProvider = {
  parseSearch
};

export async function parseSearch(input: {
  query: string;
  botMode?: KoiBotMode | "watch_events";
  context?: unknown;
  form?: unknown;
}): Promise<KoiParseResponse> {
  const query = input.query.trim();
  const requestedMode = parseRequestedBotMode(input.botMode);
  ...
  if (botMode === "events") {
    ...
    if (placeForm) {
      ...
      if (placeResponse) return placeResponse;
  ...
  if (needsCurrentLocationResolution(resolvedForm)) {
    throw new ParseSearchError("Turn on location or add a city to search nearby.", 422, {
      needsLocation: true,
      form: resolvedForm
    });
  }
  ...
  if (!placeResponse) {
    throw new ParseSearchError("Where should Koi search? Try: coffee near Hoboken, or coffee between Hoboken and Edison.", 422);
  }

  return placeResponse;
}

export class ParseSearchError extends Error {
  status: number;
  details?: Record<string, unknown>;

  constructor(message: string, status = 400, details?: Record<string, unknown>) {
    super(message);
    this.name = "ParseSearchError";
    this.status = status;
...
async function parseWithOllama(query: string): Promise<ParsedSearchIntent> {
  ...
  const response = await fetchWithTimeout(`${OLLAMA_BASE_URL.replace(/\/$/, "")}/api/chat`, {
    ...
    timeoutMessage: `Ollama parse timed out after ${OLLAMA_TIMEOUT_MS}ms.`,
...
async function parseWithGemini(query: string): Promise<ParsedSearchIntent> {
  ...
  const response = await fetchWithTimeout(
    `https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${encodeURIComponent(DEDICATED_GEMINI_KEY)}`,
    {
      ...
      timeoutMessage: `Gemini parse timed out after ${GEMINI_TIMEOUT_MS}ms.`,
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        generationConfig: {
          temperature: 0,
