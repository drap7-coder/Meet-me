...
function getMemoryStore() {
  const globalStore = globalThis as typeof globalThis & {
    __tmdbCache?: Map<string, MemoryEntry>;
  };
  globalStore.__tmdbCache ??= new Map<string, MemoryEntry>();
  return globalStore.__tmdbCache;
}
...
export async function withTmdbCache<T>(path: string, params: Record<string, string>, loader: () => Promise<T>) {
